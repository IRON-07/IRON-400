...
module.exports.run = function({ api, event, args, Currencies }) {
           let { threadID, senderID, messageID } = event;
			...
			if (!mention && sender == 'me') return api.sendMessage(`Bạn đã sửa tiền cho bản thân thành ${moneySet.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")} VNĐ`, threadID, () => Currencies.increaseMoney(senderID, parseInt(moneySet)), messageID);
			return api.sendMessage({
				body: `Bạn đã sửa tiền cho ${event.mentions[mention].replace("@", "")} thành ${moneySet.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")} VNĐ`,
				mentions: [{
